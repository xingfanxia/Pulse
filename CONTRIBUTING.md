...
Pulse is a macOS menu-bar app. There is no linter; there is a test target (`swift test`) covering rules, cache reconciliation and provider fixtures — see [Docs/testing.md](Docs/testing.md) for what it does and does not reach. Review is human plus whatever the author ran. This file is for people changing the repo; [AGENTS.md](AGENTS.md) is the short AI entry.
...
2. Change code and the **authoritative topic doc in the same patch**. Do not leave AGENTS.md as a second source of truth, and do not grow it with new architecture.
...
|---|---|---|
| Nonnegotiable constraints for agents | [AGENTS.md](AGENTS.md) — keep short | Only when a constraint is new or wrong |
...
**Current facts** are things that must stay true of *this* tree (provider count, timer shape, which accounts can be added). They belong in the topic doc, and a one-line reminder may sit at the bottom of AGENTS.md. If the code disagrees with the doc, the code wins until the doc is fixed — verify, then patch both.
...
- [ ] The topic doc, not AGENTS.md, is the source of truth for the behaviour.
...
- [ ] Links from AGENTS.md’s “Read by task” table still resolve.
