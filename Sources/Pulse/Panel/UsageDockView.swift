...
enum DockLayout {
    ...
    static func ringOffsetInItem(on axis: PanelEdge.Axis) -> CGFloat {
        ...
        return (showsPercentages(on: axis) ? percentTextHeight + ringToTextSpacing : 0) + ClauthRailCaption.height(on: axis, spacing: ringToTextSpacing)
    }
    ...
    static func itemLength(on axis: PanelEdge.Axis) -> CGFloat {
        guard showsPercentages(on: axis) else { return ringDiameter + ClauthRailCaption.height(on: axis, spacing: ringToTextSpacing) }
        return axis == .vertical ? itemHeight + ClauthRailCaption.height(on: axis, spacing: ringToTextSpacing) : max(ringDiameter, percentTextWidth)
    }
    ...
    static func thickness(on axis: PanelEdge.Axis) -> CGFloat {
        guard axis == .horizontal, showsPercentages(on: .horizontal) else { return axis == .vertical ? width + ClauthRailCaption.widening : width }
